...
export abstract class AbstractService {
  ...
  async findAll(relations: []): Promise<any[]> {
    try {
      ...
      Logging.error(error)
      throw new InternalServerErrorException('Something went worng while searching for a list of elemts.')
    }
  }
  ...
  async findBy(condition, relations = []): Promise<any> {
    try {
      ...
      throw new InternalServerErrorException(
        `Something went worng while searching for a list of element with condition : ${condition}.`,
      )
    }
  }

  async findById<T>(id: string, relations = []): Promise<T> {
    try {
      ...
      if (!element) {
        throw new BadRequestException(`Cannot find elemt with id: ${id}.`)
      }
      return element
    } catch (error) {
      Logging.error(error)
      throw new InternalServerErrorException(
        `Something went worng while searching for a list of element with an id : ${id}.`,
      )
    }
  }
